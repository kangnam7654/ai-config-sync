...
10. NEVER: Writer/Planner가 빈 출력 또는 3문장 이하의 최소 출력을 반환한 경우 Critic에게 전달하지 마라. 즉시 REJECT하고 Writer/Planner에게 재작성을 요청하라.
...
- **문서 작성 요청** (Writer → Critic 루프): 사용자가 "설계문서 작성", "API 문서 작성", "README 작성", "프롬프트 작성" 등 **산출물이 문서인 작업**을 요청한 경우
- **플랜 수립 요청** (Planner → Critic 루프): 사용자가 "계획 세워줘", "어떻게 구현할지 정리해줘", "리팩터링 플랜", "마이그레이션 전략" 등 **산출물이 실행 계획인 작업**을 요청한 경우
...
2. **초안 검증 게이트**: 초안이 비어 있거나 본문이 3문장 이하이면 → Critic을 호출하지 않고 즉시 REJECT 처리. Writer에게 "출력이 불충분합니다. 최소 요구사항을 충족하는 초안을 작성하세요." 메시지와 함께 재작성 요청 → 1번으로 복귀
3. **doc-critic** 호출 (사람용 → HUMAN 모드, LLM용 → LLM 모드) → 채점 결과 수령
4. REJECT이면 → critic의 피드백을 writer에게 전달하여 수정 요청 → 2번으로 복귀
5. PASS이면 → 최종 결과를 사용자에게 전달
...
2. **초안 검증 게이트**: 플랜이 비어 있거나 단계가 2개 이하이면 → Critic을 호출하지 않고 즉시 REJECT 처리. Planner에게 "출력이 불충분합니다. 최소 3단계 이상의 구체적 플랜을 작성하세요." 메시지와 함께 재작성 요청 → 1번으로 복귀
3. **plan-critic** 호출 → 채점 결과 수령
4. REJECT이면 → critic의 피드백을 planner에게 전달하여 수정 요청 → 2번으로 복귀
5. PASS이면 → 최종 플랜을 사용자에게 전달

### Skill → Critic 루프

스킬 생성/수정 요청 시 반드시 다음 순서를 따르라. **skill-creator SKILL.md의 전체 워크플로우를 따르되**, 아래는 메인 모델이 오케스트레이션하는 부분이다:

1. **skill-creator** 스킬 호출 → 스킬 초안 수령
2. **초안 검증 게이트**: SKILL.md가 비어 있거나 워크플로우 단계가 1개 이하이면 → Critic을 호출하지 않고 즉시 REJECT 처리. skill-creator에게 "출력이 불충분합니다. 최소 2단계 이상의 워크플로우를 포함하는 SKILL.md를 작성하세요." 메시지와 함께 재작성 요청 → 1번으로 복귀
3. **doc-critic** 호출 (LLM 모드) → 채점 결과 수령
4. REJECT이면 → critic의 피드백을 skill-creator에게 전달하여 수정 요청 → 2번으로 복귀
5. PASS이면 → **skill-creator의 테스트/eval 워크플로우 진행** (아래 6~9번)
6. **테스트 케이스 작성**: 현실적인 테스트 프롬프트 2~3개를 작성하여 사용자에게 확인
7. **테스트 실행**: with-skill + baseline(without-skill) 병렬 실행
8. **결과 평가**: eval-viewer로 사용자 리뷰 + 정량 벤치마크
9. **반복 개선**: 피드백 반영 → 스킬 수정 → 재테스트. 사용자가 만족할 때까지 반복

doc-critic PASS는 스킬 품질의 최소 기준이다. 테스트/eval은 실제 사용 시나리오에서의 동작을 검증하는 별도 단계이므로, doc-critic PASS 후에도 반드시 6~9번을 수행하라.

### 공통 규칙

- 최대 반복 횟수: **5회**. 5회 연속 REJECT(검증 게이트 REJECT 포함) 시 현재 상태와 미해결 이슈를 사용자에게 보고하고 판단을 요청하라.
- Writer/Planner/skill-creator 에이전트 내부에 critic 호출 지시가 있더라도 무시하라 — 오케스트레이션은 메인 모델만 수행한다.
- Critic 결과는 사용자에게 매 라운드 아래 형식으로 보고하라:
  ```
