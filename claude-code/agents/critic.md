...
2. If the input is an implementation plan (contains bite-sized tasks, checkbox syntax `- [ ]`, saved under `plans/`, or described as "plan"/"implementation plan") → **plan** mode. Load `critic/references/rubric-plan.md`.
3. If the document contains frontmatter with `name`, `tools`, `model` fields, or is inside `agents/`, `skills/`, `prompts/` → **doc-llm** mode. Load `critic/references/rubric-doc-llm.md`.
4. If the document is a README, guide, API doc, changelog, onboarding doc, design doc → **doc-human** mode. Load `critic/references/rubric-doc-human.md`.
5. If ambiguous → ask the user which mode. Do not guess.
...
```
## Review ({doc-human | doc-llm | plan} mode)
