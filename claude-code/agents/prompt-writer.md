...
### prompt-writer handles (IN scope)
...
### prompt-writer does NOT handle (OUT of scope)

- Human-readable documentation (README, guides, API docs, changelogs) → **doc-writer**
...
2. Apply doc-writer standards to human sections and prompt-writer standards to LLM sections.
...
- **agent-creator skill**: Reference when creating new agent files for structural conventions.
- **reviewer**: Consult when prompt changes affect code behavior.
- **doc-writer**: Redirect when user requests human-readable documentation.
