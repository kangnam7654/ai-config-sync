...
- 코드베이스 경로 (필수)
- 플래그 (선택): auto-improve에서 전달받은 영역 선택/확장 플래그. 미지정 시 기본 7개 전체 진단.
  - 영역 선택: `--code`, `--security`, `--arch`, `--db`, `--test`, `--repo`, `--ui` → 해당 영역만 진단
  - 확장: `--bm` → BM 진단 추가, `--design` → 심화 디자인 리뷰 추가 (`--ui` 자동 포함)
...
audit_scope:
  ...
  bm: false          # --bm 플래그 시 true
  design: false      # --design 플래그 시 true (ux_ui도 true로 강제)
flags: []  # auto-improve에서 전달받은 플래그 목록
```

**플래그 → scope 매핑 규칙**:
- 영역 선택 플래그(--code 등)가 1개 이상 → 해당 영역만 true, 나머지 false (자동감지 무시)
- 영역 선택 플래그 없음 → 기본 동작 (전체 + 자동감지)
- `--bm` → `bm: true` 추가
- `--design` → `design: true` + `ux_ui: true` 강제
...
#### #8.5 BM 진단 ⓟ [조건부: `--bm` 플래그 시]

**에이전트**: bm-designer

기존 BM의 존재 여부를 파악하고, 있으면 진단한다. BM 존재 판별: 결제/구독/과금 관련 코드, pricing 페이지, 수익 모델 문서 등을 탐색.

**프롬프트 템플릿**:
```
아래 코드베이스의 비즈니스 모델 상태를 진단하라.
프로젝트 경로: {project_path}
기술 스택: {tech_stack}

1단계: BM 존재 여부 판별
- 결제/구독/과금 코드 (Stripe, PayPal, IAP 등)
- pricing 페이지 또는 플랜 정의
- 수익 모델 관련 문서
...
출력 형식:
- bm_exists: true/false
- (bm_exists=true 시) 점수 + 발견 목록 (#2와 동일 형식)
- (bm_exists=false 시) "BM 미존재" 기록
```

**산출물**: BM 진단 결과 (bm_exists 플래그 포함). auto-improve의 Phase 1.5 BM 라우팅에서 이 결과를 참조한다.

---

#### #8.6 심화 디자인 리뷰 ⓟ [조건부: `--design` 플래그 시]

**에이전트**: designer

#8 UX/UI 진단보다 심화된 시각 디자인 품질을 진단한다. #8이 "동작하는가?"를 본다면, #8.6은 "아름다운가?"를 본다.

**프롬프트 템플릿**:
```
아래 코드베이스의 시각 디자인 품질을 심층 진단하라.
프로젝트 경로: {project_path}
프론트엔드 스택: {frontend_stack}

진단 항목:
- 타이포그래피 체계 (폰트 선택, 크기 스케일, line-height, 자간)
- 색상 시스템 (팔레트 일관성, 대비, 브랜드 적합성)
- 간격/여백 시스템 (spacing scale 사용 여부, 일관성)
- 레이아웃 그리드 (정렬, 비율, 반응형 breakpoint)
- 컴포넌트 스타일 통일성 (버튼, 카드, 인풋 등)
- 아이콘/일러스트 일관성
- 현재 디자인 트렌드 적합성
...
### #9 CTO 종합 판정 + Audit Report 문서화

**에이전트**: CTO → doc-loop

모든 병렬 진단(#2~#8) 결과를 수집한 뒤 실행한다. CTO 판정과 문서화를 한 단계로 수행한다.
...
baseline_scores:
  ...
  repo_health: 5.0
  ux_ui: 6.0       # 또는 N/A
  bm: 3.0          # --bm 시에만, 또는 N/A. bm_exists: false면 0
  design: 5.0      # --design 시에만, 또는 N/A
priority_items:
...
**CTO 판정 완료 후**, gate-decision.yaml이 PROCEED 또는 PARTIAL이면 **doc-loop 스킬**을 자동(B) 모드 + LLM 모드로 호출하여 Audit Report를 문서화한다. SKIP이면 간략 보고 후 종료.

**입력**: #1~#8의 모든 결과 (scope-map, 진단 결과 7종, gate-decision.yaml)
...
| #9 문서화 (doc-loop) | doc-loop 내부 5라운드 기준 적용 |
