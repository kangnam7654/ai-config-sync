...
│                auto-improve-loop                 │
│                                                  │
│  Round 1: auto-improve (Full 37단계)             │
...
- 목표 점수 (선택, 기본값: 8.0)
- 최대 라운드 (선택, 기본값: 5)
- 집중 영역 (선택): 특정 영역만 반복 개선 (예: "보안과 테스트만")
...
target_score: 8.0
max_rounds: 5
focus_areas: null  # 또는 [security, test] 등
...
집중 영역: {focus_areas 또는 "전체"}
```

#### 라운드 2+ (반복)

이전 라운드의 결과를 auto-improve에 전달하여 중복 작업을 방지한다.
