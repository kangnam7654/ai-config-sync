...
plan-loop는 내부에서 planner↔plan-critic을 **최대 5라운드** 반복한다. plan-loop가 PASS를 반환하면 #25로 진행한다.

plan-loop가 FAIL을 반환하면 (5라운드 소진 또는 plan-critic이 해결 불가로 판단), design-loop가 plan-critic의 피드백 내용을 분석하여 실패 유형을 판별하고 해당 서브-루프를 재실행한다:
