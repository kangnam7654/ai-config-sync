...
def run_new(title: str, project: str, priority: str | None,
            tags: str | None, due: str | None) -> tuple[bool, str, str]:
    args = [title, "--project", project]
...
def run_set(card_id: str, fields: dict) -> tuple[bool, str, str]:
    args = [card_id]
    for k in ("title", "priority", "tags", "due", "epic", "project"):
        v = fields.get(k)
        if v is None or v == "":
            continue
        args += [f"--{k}", str(v)]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_new(self) -> None:
        ...
        ok, stdout, stderr = run_new(title, project,
...
async function handleCreate() {
  ...
  try {
    const r = await fetch("/api/new", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, project, priority, tags, due }),
    });
    const j = await r.json();
    if (!r.ok || !j.ok) throw new Error(j.error || `HTTP ${r.status}`);
    showToast("Created");
