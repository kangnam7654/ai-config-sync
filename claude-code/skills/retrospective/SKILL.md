...
**REQUIRED BACKGROUND:** 공통 loop 패턴은 `~/.claude/skills/_shared/loop-pattern.md` 참조.
...
> "통합 retro(기간 기반, 여러 프로젝트)와 마일스톤 retro(특정 프로젝트의 버전/기능 완료) 중 어느 쪽인가요?"
...
**3-C. milestone retro 미커버 영역의 raw git log (보조 입력)**
```bash
...
### 마일스톤 retro (mode = `milestone`)

사용자에게 확인 (1회, 3개 항목):
- 프로젝트명·경로
- 마일스톤 라벨 (예: `v0.0.4 — 청산 마일스톤`)
...
- mode: <integrated | milestone>
- period: <start_date> ~ <end_date>
- project_name: <…>          # milestone 모드만
- milestone_label: <…>        # milestone 모드만
- round: 1
...
mkdir -p ~/wiki/Retro/milestones/<프로젝트>   # 마일스톤
```

파일명:
- 통합: `~/wiki/Retro/regular/YYYY-MM-DD.md` (종료 날짜)
- 마일스톤: `~/wiki/Retro/milestones/<프로젝트>/<슬러그>-YYYY-MM-DD.md`

커밋:
```bash
git -C ~/wiki add Retro/regular/YYYY-MM-DD.md     # 또는 milestones/...
git -C ~/wiki commit -m "retro: <start_date> ~ <end_date>"     # 또는 retro(milestone): ...
```

`Rules/`는 add 안 함 (Rule 업데이트 제안은 사용자 승인 후 별도 작업). push 안 함 (NEVER #2).
...
| 통합 retro에서 milestone retro도, 직전 통합 retro도 없음 (첫 회고) | data_bundle에 raw git log만 채워 runner 호출. critic은 동일 기준 적용 |
