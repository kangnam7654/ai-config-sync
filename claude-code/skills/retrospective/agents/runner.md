...
전달받은 데이터(이전 milestone retros, 직전 통합 retro의 Action Items, raw git log,
...
- **mode**: `integrated` (통합 retro) 또는 `milestone` (마일스톤 retro)
- **period**: `start_date` ~ `end_date` (또는 milestone 시작 commit ~ HEAD)
- **project_name**: milestone 모드에서만. 프로젝트 식별자.
- **milestone_label**: milestone 모드에서만. 예: `v0.0.4 — 청산 마일스톤`
- **data_bundle**: 다음 항목들의 _이미 수집된_ 결과
  - `prior_milestone_retros`: 기간 내 작성된 milestone retro 파일들의 본문 (integrated 모드)
  - `prior_integrated_retro_actions`: 직전 통합 retro의 Action Items 섹션 + 처리 여부 (integrated 모드)
  - `raw_git_log`: milestone retro가 커버하지 않는 영역의 commit list (integrated 모드 보조 입력)
  - `project_git_log`: 해당 프로젝트의 commit list + 주요 diff stat (milestone 모드)
...
**integrated 모드:**
1. 1차 입력: `prior_milestone_retros` 내 4L 항목들. 그대로 인용·요약하지 말고
...
3. 보조 입력: `raw_git_log`. milestone retro가 커버하지 않는 영역만 직접 분석.
   주제별로 그루핑.
...
```markdown
# Milestone Retro: {project_name} — {milestone_label}
...
- Longed For 중 다음 마일스톤에서 우선 처리할 것 (구체적 카드 단위)
```
...
- milestone 모드: 다음 마일스톤 메모가 추상적 다짐이 아니라 _카드/트랙 단위_
...
- integrated 모드에서 milestone retro가 있으면 _그것을 1차 입력으로_, raw git log는 보조
