...
`travel` 스킬의 Transport Search 모드로 교통편 실시간 가격을 조회한다.
...
### Transport Search 실패
교통편 조회가 실패하거나 결과가 없으면 `WebSearch "{출발지} to {목적지} flight/train"` 으로 대략적 가격을 제시하고, 해당 항목에 "[transport-search 미사용 추정치]" 라벨을 부착한다.
