---
name: ui-review
description: "Screenshot-based UI scoring against current design trends. Covers web apps, mobile apps, slides, and any visual interface. Scores hierarchy, consistency, trend fitness, responsiveness, accessibility."
---
...
iOS 시뮬레이터에서 실행 중인 앱의 UI를 리뷰할 때 사용한다. 이 스킬은 simulator 에이전트를 직접 호출하지 않는다 (NEVER 규칙 #10: 서브에이전트 간 직접 호출 금지). 대신 메인 모델에게 스크린샷 캡처를 요청한다.
...
simulator 에이전트를 스폰하여 다음을 수행해주세요:
...
메인 모델이 simulator 에이전트를 통해 스크린샷을 캡처한 후, 해당 이미지 경로를 받아 Step 2부터 진행한다.

**여러 화면 리뷰가 필요한 경우:** 메인 모델에게 simulator 에이전트가 Maestro 플로우로 여러 화면을 순회하며 각 화면의 스크린샷을 캡처하도록 요청한다.
...
| iOS Simulator app but no simulator booted | Return to main model: "시뮬레이터가 부팅되어 있지 않습니다. simulator 에이전트로 시뮬레이터를 부팅하고 앱을 실행한 후 스크린샷을 캡처해주세요." |
| iOS Simulator app but simulator agent unavailable | Fall back to manual: "xcrun simctl io booted screenshot /tmp/ui-review-sim.png 으로 직접 스크린샷을 캡처해주세요." |
| Multiple iOS Simulator screens to review | Request main model to spawn simulator with a Maestro flow that navigates screens and captures screenshots at each step. |
