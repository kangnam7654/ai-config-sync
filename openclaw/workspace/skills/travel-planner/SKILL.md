...
조회 방법:
- `WebSearch "{destination} events {month} {year}"` — 현지 이벤트·축제 검색
- `WebSearch "{destination} weather {month}"` — 날씨·시즌 정보 검색
- `WebFetch` — 검색 결과 중 유용한 페이지 상세 읽기
- `agent-browser` — 동적 SPA나 로그인 필요 페이지에서만 사용
